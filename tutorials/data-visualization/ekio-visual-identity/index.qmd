...
tbl_growth <- tibble(
  year = 2020:2024,
  Agriculture = c(2.1, -1.2, 3.4, 1.8, 2.3),
  Industry = c(-8.7, 4.5, 2.1, 3.2, 1.9),
  Services = c(-7.8, 8.2, 4.1, 2.8, 3.1)
)

tbl_growth <- tbl_growth |>
  tidyr::pivot_longer(
    cols = -year,
    names_to = "sector",
    values_to = "employment_growth")

sector_data <- left_join(sector_data, tbl_growth, by = c("year", "sector"))
