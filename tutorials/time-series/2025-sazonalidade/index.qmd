...
temp = readr::read_rds(here::here("static/data/tutorials/temperatura_poa_2023.rds"))
gasolina = readr::read_csv(here::here("static/data/tutorials/br_anp_gasolina.csv"))
```
...
Vou também definir um tema padrão para os gráficos usando o pacote `{ekioplot}`. Este passo é totalmente opcional, mas gera gráficos de maior qualidade.

```{r}
library(ekioplot)

theme_series = theme_ekio() +
  theme(
...
ggplot2::theme_set(theme_series)
update_geom_defaults(
  "point",
  list(color = ekioplot::ekio_colors("modern_premium")[7])
)
update_geom_defaults(
  "line",
  list(color = ekioplot::ekio_colors("modern_premium")[7])
...
O [teste de Kruskal-Wallis](https://en.wikipedia.org/wiki/Kruskal–Wallis_one-way_analysis_of_variance) (KW) verifica se amostras distintas foram geradas pela mesma distribuição. Para detectar sazonalidade em séries mensais, adaptamos este teste da seguinte forma: cada mês "fornece" observações ao longo de vários anos e queremos verificar se existe uma diferença sistemática entre os meses. Assim, temos 12 amostras (12 meses) com n observações cada (onde n é o número de anos na série).

O teste consiste em:

1. Ranquear todas as observações da amostra;
2. Calcular a soma dos ranques dentro de cada grupo (e.g. dentro de cada mês);
3. Verificar se estes valores calculados se aproximam dos valores que esperaríamos obter caso os dados fossem puramente aleatórios.

Vale notar que o teste deve preferencialmente ser aplicado sobre dados sem tendência, especialmente quando a série apresenta tendência forte. Séries com tendência podem gerar falsos positivos, pois os valores mais recentes serão sistematicamente maiores que os mais antigos, independentemente da sazonalidade. Aqui, utilizo a série livre de tendência obtida pelo filtro MA 12x2.

O códiogo prepara os dados para o teste.
...
Para ganhar um pouco de intuição, podemos empilhar os meses em uma tabela.
...
```{r}
# Summary by month to show the pattern
monthly_summary = tab |>
  dplyr::group_by(mes) |>
  dplyr::summarise(
    mean_rank = mean(rank),
