...
if (interactive()) {
  ...
  .GlobalEnv$ekio_colors <- list(
    ...
    sapphire_seq = c("#f4f7fb", "#e3ecf5", "#d2e1ef", "#a0bfdb", 
  ...
  suppressMessages({
    if(require(tidyverse, quietly = TRUE)) {
      ...
      if(require(ggplot2, quietly = TRUE)) {
        theme_set(theme_minimal(base_size = 12, base_family = "Avenir"))
        update_geom_defaults("point", list(color = .GlobalEnv$ekio_colors$modern_premium))
        update_geom_defaults("line", list(color = .GlobalEnv$ekio_colors$modern_premium))
      }
    }
    
    # Load Brazilian data packages if available
    if(require(geobr, quietly = TRUE)) {
      cat("🗺️  geobr package loaded for Brazilian spatial data\n")
    }
    
    if(require(sidrar, quietly = TRUE)) {
      cat("📊 sidrar package loaded for IBGE data access\n")
    }
    
    if(require(GetBCBData, quietly = TRUE)) {
      cat("🏦 GetBCBData package loaded for Banco Central data\n")
    }
  })
  ...
  cat("   • Academy Primary (Educational): ", .GlobalEnv$ekio_colors$academy_primary, "\n")
  cat("   • Modern Premium (Professional): ", .GlobalEnv$ekio_colors$modern_premium, "\n")
  cat("   • Academic Authority (Research): ", .GlobalEnv$ekio_colors$academic_authority, "\n\n")
  
  cat("💡 Pro Tip: Use 'source(\"assets/r/ekio-themes.R\")' for complete theme functions\n\n")
}